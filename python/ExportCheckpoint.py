...
class GoNet(nn.Module):
    def __init__(self):
        ...
        # 第一层卷积：输入通道 4，输出通道 256，卷积核大小 5x5，步幅 1，填充 2（保持尺寸）
        self.conv1 = nn.Conv2d(in_channels=4, out_channels=512, kernel_size=5, stride=1, padding=2)
        # 后续 11 层卷积：输入通道 256，输出通道 256，卷积核大小 3x3，步幅 1，填充 1（保持尺寸）
        self.hidden_convs = nn.Sequential(
            *[nn.Conv2d(in_channels=512, out_channels=512, kernel_size=3, stride=1, padding=1) for _ in range(21)]
        )
        # 输出层：1x1 卷积，输入通道 256，输出通道 1（对应棋盘每个位置的概率）
        self.output_conv = nn.Conv2d(in_channels=512, out_channels=1, kernel_size=1, stride=1, padding=0)
        # 激活函数
        self.relu = nn.ReLU()
